...
namespace Engine.Actions
{
    public class AttackWithWeapon
    {
        ...
        public void Execute(LivingEntity actor, LivingEntity target)   // command that accepts two parameters 1) actor (who is performing the action) 2) target (who is having the action done to them)
        {
            ...
            {
                ReportResult($"You hit the {target.Name.ToLower()} for {damage} points."); // message before taking damage to prevent situation were OnKilled event on monster raising before showing damage (want to be sure that damage msg will appear before killed info)
