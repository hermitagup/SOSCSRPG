...
namespace Engine.Factories
{
    public static class TraderFactory
    {
        ...
        { //static function to look in trader list by name and check if it matches (first one) and return that
            return _traders.FirstOrDefault(t => t.Name == name);
        }

        private static void AddTraderToList(Trader trader)
        { //check if trader list has a trader name (each trader has an unique name)
